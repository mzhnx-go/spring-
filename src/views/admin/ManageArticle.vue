...
function deleteArticle() { // 删除文章
  axios({
    ...
    if (response.data.success) {
      ...
      dialogVisible.value = false // 可选：删除失败后也关闭对话框
    }
  }).catch((error) => {
    ElMessageBox.alert("系统错误！", '结果')
    dialogVisible.value = false // 添加这行：出错后关闭对话框
  })
}
</script>
<template>
  ...
  <el-row justify="center" align="middle" style="height:60px">
    <el-col :span="16">
      <el-pagination v-model:current-page="myData.pageParams.page" v-model:page-size="myData.pageParams.rows"
        ...
        @size-change="handleSizeChange" @current-change="handleCurrentChange" :pager-count="7" background />
    </el-col>
  </el-row>
